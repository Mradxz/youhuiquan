...
;(function($){
	//提示
	if('close'==$.yangtata.util.getCookie('leftimg')){
	...
	$('.iknow').live('click', function(){
		$('.tips_goods').css('display','none');
		$.yangtata.util.setCookie('leftimg', 'close');
		$('.tips_goods').remove();
	});
    ...
    function() {
        ...
        if (flag == true && $(document).scrollTop() > temp) {
            ...
            if ("fold" == $.yangtata.util.getCookie("fold")) {
    ...
    function() {
        ...
        $('.white_dh #fold').attr('class', 'filter-unfold');
        $.yangtata.util.setCookie("fold", 'fold');
    });
	...
	var F_s = function() {
			if (document.getElementById("k")) {
				document.getElementById("k").onclick = function() {
					...
					var ekv = yangtataER.site +'/?m=search&a=etao&key=' + val;
					window.location.href = yangtataER.site +'/?m=search&a=index&key=' + val;
					return false;
				}
			}
