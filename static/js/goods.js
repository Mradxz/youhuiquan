...
;(function($){
    $.yangtata.goods = {
        init: function(options){
            $.yangtata.goods.goods_add();
			$.yangtata.goods.mygoods();
        },
        ...
        goods_add: function(){
             $(".txt:first").focusout(function() {
				  var inform_html = '<li id="detail">' 
					...
					if (link != '') {
						 $.getJSON(yangtataER.root + '/?m=goods&a=ajaxGetItem', {url:link}, function(result){
							if(result.status == 1){
								if ($('#detail').size() != 0) {
									$('#detail').remove();
								}
			...
			$("#smt").click(function() {
				 ...
				 var textnum = $.yangtata.util.getStrLength(intro);
				...
				$.ajax({
                    url: yangtataER.root + '/?m=goods&a=ajaxadd',
                    type: 'POST',
                    data: {
                        iid: iid,
                        cate_id: cate_id,
