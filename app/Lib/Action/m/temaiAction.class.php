...
class temaiAction extends FirstendAction {
	...
	public function index(){
		...
		if($cid){
		$ajaxurl = C('ftx_header_html').'?m=temai&a=index&cid='.$cid.'&sort='.$sort.'&mode='.$mode;
		...
		$ajaxurl = C('ftx_header_html').'?m=temai&a=index&sort='.$sort.'&mode='.$mode;
		$this->assign('ajaxurl',$ajaxurl);		
		$this->_config_seo(C('ftx_seo_config.cate'), array(            
            'seo_title' =>'特卖精选',
			'seo_keywords' => '精选特卖',
