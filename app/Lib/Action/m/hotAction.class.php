<?php
class hotAction extends FirstendAction {
	...
	public function index(){	
		...
		if($cid){
		$ajaxurl = C('ftx_header_html').'?m=hot&a=index&cid='.$cid.'&sort='.$sort.'&mode='.$mode;
		...
		$ajaxurl = C('ftx_header_html').'?m=hot&a=index&sort='.$sort.'&mode='.$mode;
		$this->assign('ajaxurl',$ajaxurl);				
		$this->_config_seo(C('ftx_seo_config.cate'), array(            
            'seo_title' =>'超级人气榜',
			'seo_keywords' => '人气榜，点击榜',
			'seo_description' =>'超级人气榜，按用户点击量从高到低排序。',
        ));	
		}  
		$this->assign('p', $p);	
		$this->display();
