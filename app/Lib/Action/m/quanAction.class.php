...
class quanAction extends FirstendAction {
	...
	public function index(){
		...
		if($cid){
		$ajaxurl = C('ftx_header_html').'?m=quan&a=index&cid='.$cid.'&sort='.$sort.'&mode='.$mode;
		...
		$ajaxurl = C('ftx_header_html').'?m=quan&a=index&sort='.$sort.'&mode='.$mode;
		$this->assign('ajaxurl',$ajaxurl);		
		$this->_config_seo(C('ftx_seo_config.cate'), array(            
            'seo_title' =>'领券优惠直播',
			'seo_keywords' => '优惠券',
			'seo_description' =>'先领券再购买，让你购物省了又省。',
        ));	
		}        
		$this->assign('p', $p);	
		$this->display();
